...
pub fn executeable<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, Executeable, E> {
    alt((command_executeable, call_executeable, sh_block_executeable))(i)
}
...
fn sh_block_executeable<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&str, Executeable, E> {
    let (i, _) = multispace0(i)?;
    let (i, output_variables) = opt(output_variable_bindings)(i)?;
    let (i, _) = preceded(space0, tag("sh"))(i)?;
    let (i, name) = opt(executor_name)(i)?;
    let (i, _) = preceded(space0, char(':'))(i)?;
    ...
    Ok((
        ...
        Executeable {
            ...
            name,
            options: None,
            exec_type: ExecuteableType::ShBlock { execs },
        },
    ))
}
...
mod tests {
    ...
    mod sh_block {
        ...
        fn ok_simple() {
            assert_eq!(
                executeable::<Error<&str>>("sh: {run: test;};"),
                Ok((
                    "",
                    Executeable {
                        output_variables: None,
                        name: None,
                        options: None,
                        exec_type: ExecuteableType::ShBlock {
        ...
        fn ok_named() {
            assert_eq!(
                executeable::<Error<&str>>("sh as pre: {run: test;};"),
                Ok((
                    "",
                    Executeable {
                        output_variables: None,
        ...
        fn ok_nested() {
            assert_eq!(
                executeable::<Error<&str>>("sh as pre1: {sh as pre2: {run: test;};};"),
                Ok((
                    "",
                    Executeable {
                        output_variables: None,
            ...
            assert_ne!(
                ...
                Ok((
                    ...
                    Executeable {
                        ...
                        name: None,
                        options: None,
                        exec_type: ExecuteableType::ShBlock { execs: vec![] }
                    }
                ))
            );
        }
